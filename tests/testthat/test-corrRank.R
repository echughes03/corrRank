...
test_that("two predictor ranking works", {
  ...
  expect_equal(output$Correlation[1] <= output$Correlation[2])
})
...
test_that("three predictor ranking works", {
  ...
  expect_equal(output$Correlation[1] <= output$Correlation[2] & output$Correlation[2]<= output$Correlation[3])
})
