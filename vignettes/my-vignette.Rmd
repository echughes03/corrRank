...
The function outputs a predictor-outcome scatter plot for each predictor vector in the list of predictors. The order of scatter plot output is determined by the order of predictor vectors listed. A line of best fit and pearson correlation coefficient accompanies the correlation plot.

From the `cities` data set, we can visualize the correlation for a list of predictors: average hours worked annually (`avg_hours_worked_annual`), pollution (`pollution_index`) and number of hours of sunshine per year (`sunshine_hours`), against the outcome, 'happiness level'. Using `cplot()` we write:
...
Individual scatter plots can be extracted for each predictor using the order that they are listed in. In the `cities` data set, `cplot()` takes the list of predictors `exposures` which consists of (1) average hours worked annually, (2) pollution, and (3) annual hours of sunshine, embedded in that order. To extract individual scatter plots, we write:
